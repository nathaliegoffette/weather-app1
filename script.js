...
const APIResponse = document.querySelector('#APIResponse');
    ...
    bSubmit.addEventListener('click', function() {
        ...
        //let array1 = json.list[1].dt;

        //const resultat = array.find( day => day.dt);
       ...
       //console.log(array1);
        ...
        container.innerHTML = list;
          ...
          //let { temp } = json.main;
          //let { place } = json.name;
          let { description, icon } = json.weather[0];
          let { sunrise, sunset } = json.sys;
          let iconUrl = `https://openweathermap.org/img/wn/${icon}@2x.png`;
         
          // Converting Epoch(Unix) time to UTC
          let sunriseUTC = new Date(sunrise * 1000);
          let sunsetUTC = new Date(sunset * 1000);
          let whatDay = document.querySelector('#day');
          ...
          whatDay.textContent = json.dt;
          desc.textContent = `${description}`;
          tempC.textContent = `${temp.toFixed(2)} °C`;
          sunriseDOM.textContent = `${sunriseUTC.toLocaleDateString()}, ${sunriseUTC.toLocaleTimeString()}`;
          sunsetDOM.textContent = `${sunsetUTC.toLocaleDateString()}, ${sunsetUTC.toLocaleTimeString()}`;
        */
        });
 ...
 /*
      ...
      let htmlCode = ``;
          list.forEach(function(getAllDays) {
         htmlCode =
         htmlCode +
          `
          <article>
           <div class="day"><h2>${dayTime}</h2>
                 <img src="" alt="" srcset="" id="weather-icon">
                <div id="location">${place}</div>
                   <div class="desc">${description}</div>
                   <div class="weather">
                   <div class="c">${temp.toFixed(2)} °C</div>
                   </div>
                   <div class="info">
                       <h4>Sunrise: <span class="sunrise">${sunriseUTC.toLocaleDateString()}, ${sunriseUTC.toLocaleTimeString()}</span></h4>
                      <h4>Sunset: <span class="sunset">${sunsetUTC.toLocaleDateString()}, ${sunsetUTC.toLocaleTimeString()}</span></h4>
                   </div>
               </div>
            </article>
         `;   
          const dayCards = document.querySelector("container");
     dayCards.innerHTML = htmlCode;
*/